(function() {
	...
	siteAlbum.directive('siteAlbum', function($http, $location, $rootScope, $timeout, pageTitle) {
		...
		function link(scope, element, attrs) {
			var url = $location.path().substr($location.path().indexOf('/photos/') + 8, $location.path().length),
				hash = $location.hash().substr($location.hash().indexOf('photo') + 5, $location.hash().length);
					...
					$timeout(function() {
						...
						slider.flexslider({
							...
							after: function() {
								if ( element.hasClass('fullsize') ) {
									$location.hash($('.flex-active-slide').attr('id'));
	
									var breadcrumbs = [{
										title: 'Фотографии',
										url: '/photos'
									}, {
										title: scope.album.title + '#' + $location.hash(),
										url: '/photos/' + scope.album.url + '#' + $location.hash()
									}];
						...
						if ( hash ) {
							...
							var breadcrumbs = [{
								...
								url: '/photos/' + scope.album.url
							}];
							$rootScope.$broadcast('create breadcrumbs', breadcrumbs);
						}
					}, 100);

					scope.imgFullsize = function(id) {
						if ( !element.hasClass('fullsize') ) {
